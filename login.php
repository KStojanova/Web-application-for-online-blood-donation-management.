...
$db = mysqli_connect('localhost', 'root', '', 'blood');
   ...
   if(isset($_POST['email']) && isset($_POST['password'])) {
      ...
      $query1=$db->query("SELECT * FROM admin WHERE email ='$email'");
      if (mysqli_num_rows($query1)>0){
        $adata=mysqli_fetch_array($query1);
       // if(password_verify($password, $adata['password'])){
         ...
         // }  
          //else {
             ...
             ?>
            <!-- <script language="javascript" type="text/javascript">
               alert("Грешна емаил адреса или лозинкаaaaaaaaa");
      ...
      $query2=$db->query("SELECT * FROM krvodaritel WHERE email ='$email'");
      if( mysqli_num_rows($query2) >0){
      $adata=mysqli_fetch_array($query2);
